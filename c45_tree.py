...
represented by a dictionary with:
    ...
    'samples_perc':  Percentage of observations from original DF contained in this node
    'samples': Number of observations contained in this node
...
def createTree(df, targetFeature, selectedFeatures, create_btn, delete_btn, forceLeaf_btn,
    ...
    chosenAttr = None
    threshold = None
    samples_perc = round((len(df)/train_length)*100, 1)
    samples = len(df)
...
def splitNodeBy(tree, nodeId, chosenAttr, create_btn, delete_btn, forceLeaf_btn, resetLeaf_btn):
    ...
    leftSamples_perc = round((len(splitted[0])/train_length)*100, 1)
    leftSamples = len(splitted[0])
    ...
    rightSamples_perc = round((len(splitted[1])/train_length)*100, 1)
    rightSamples = len(splitted[1])
    ...
    nodeRight = {
            ...
            'samples_perc': rightSamples_perc,
