...
from my_functions import parse_file, split_train_test, get_color_scale, lda, make_figure, executeClassifictaionSklearn, \
                         lda_to_vis, split_train_test_vis
...
def train_test_dict_common(selectedFeatures, trainValue, jsonified_data, targetFeature, seed,
    ...
    if jsonified_data is not None and targetFeature is not None:
        ...
        X_train, y_train = split_train_test_vis(df, targetFeature, trainValue, seed)
        ...
        return json.dumps(train_test_dict_vis)


def execute_lda_vis(train_test_json, targetFeature, features, seed, complete_json, num_classes):
    ...
    if train_test_json is not None:
        ...
        lda_dict_vis = {}
        lda_dict_vis['color_scale'] = color_scale

        Xr_train, axis, num_classes = lda_to_vis(X_train, y_train, features, seed, num_classes)
        ...
        return json.dumps(lda_dict_vis)
...
def generate_selectedFeatures_options(options):
...
def execute_lda(train_test_json, targetFeature, features, seed, complete_json):
    ...
    if train_test_json is not None:
        ...
        Xr_train, Xr_test, axis, num_classes = lda(X_train, X_test, y_train, y_test, features, seed, None)
...
@app.callback(Output('lda-plot', 'figure'),
              ...
              [State('tree-dict', 'children'),
               ...
               State('target-feature', 'value'),
               State('seed', 'children')])
def make_lda_plot(lda_json, selectedNode_data, tree_jsonified, selectedFeatures, trainValue, jsonified_data,
                  targetFeature, seed):
                        ...
                        if node['id'] == selectedNode_data['id']:
                            if not node['isRoot']:
                                ...
                                if num_classes == 1:
                                    ...
                                    lda_dict_vis_json = execute_lda_vis(train_test_dict_jsonified, targetFeature, selectedFeatures,
                                                                           seed, jsonified_data, num_classes)
                                    # We send the original classes list to get original rgba codes
                                    classes = tree['data']['classes']
                                    figure = make_figure(lda_dict_vis_json, classes)
                                    return figure

        figure = make_figure(lda_json, None)
        return figure
    else:
        return {}
...
@app.callback(Output('table', 'children'),
              ...
              [State('tree-dict', 'children'),
               ...
               State('target-feature', 'value'),
               State('seed', 'children')])
def make_table(selectedNode_data, tree_jsonified, selectedFeatures, trainValue, jsonified_data,
                  targetFeature, seed):
    if selectedNode_data is not None:
        if len(selectedNode_data['id']) != 0:
            tree = json.loads(tree_jsonified)
                ...
                if node['id'] == selectedNode_data['id']:
                    if not node['isRoot']:
                        ...
                        if num_classes == 2:
                            ...
                            lda_dict_vis_json = execute_lda_vis(train_test_dict_jsonified, targetFeature, selectedFeatures,
                                                                   seed, jsonified_data, num_classes)
                            data_dict = json.loads(lda_dict_vis_json)
            ...
            for node in tree['nodes']:
                if node['id'] == selectedNode_data['id']:
                    ...
                    children = [
                        ...
                        html.Div(children=[
                            html.Div("Samples: ", style={'font-weight': 'bold', 'display': 'inline'}),
                            html.Div(children=[samples], style={'display': 'inline'})
                        ]),
                        html.Div(children=[
